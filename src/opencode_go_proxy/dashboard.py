...
<body>
  <main>
    ...
    <section aria-labelledby="routing-title">
      ...
      <p class="fallback-config"><span>Fallback setting</span><code id="fallback-setting">OPENCODE_CAPABILITY_MODEL=gpt-5.6-luna</code></p>
    </section>
...
function configText(){const service=state.services[Number(el("service").value)]||state.services[0];if(!service)return "";const model=el("model").value||"gpt-5.6-luna";const provider=`opencode-${slug(service.upstream)}`;const profile=`${model}-${slug(service.upstream)}`;return `[model_providers.${provider}]\nname = "OpenCode ${service.upstream}"\nbase_url = "${service.public_url}"\nexperimental_bearer_token = "local-proxy"\nwire_api = "responses"\n\n[profiles."${profile}"]\nmodel_provider = "${provider}"\nmodel = "${model}"\napproval_policy = "untrusted"\nsandbox_mode = "workspace-write"\nfeatures = { memories = false }\n`;}
function renderRoute(){const service=state.services[Number(el("service").value)]||state.services[0];if(!service)return;const model=el("model").value||"gpt-5.6-luna";const capability=el("capability").value;const routing=service.routing||{};const native=(routing.native_models||[]).includes(model);const supported=(routing.capability_models?.[model]||[]).includes(capability);let target=model,mode=native?"Native Responses":"Minimal Chat bridge",note=native?"The request is passed through unchanged.":"Only the Responses and Chat Completions shapes are translated.";if(capability!=="plain"&&!supported){if(capability==="image"&&routing.vision_bridge_model){target=`${routing.vision_bridge_model} → ${model}`;mode="Vision bridge";note="MiMo describes the current image, then the selected model continues the tool turn.";}else{target=routing.fallback_model||"gpt-5.6-luna";mode="Native capability fallback";note="The original request moves intact to Luna, preserving native tool items and citations.";}}el("route-source").textContent=model;el("route-target").textContent=target;el("route-mode").textContent=mode;el("route-note").textContent=note;el("fallback-setting").textContent=`OPENCODE_CAPABILITY_MODEL=${routing.fallback_model||"gpt-5.6-luna"}`;}
